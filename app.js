...
/* ===== GDRIVE LINKS — 6 folder laporan bulanan, admin bisa edit ===== */
var DRIVE_LINKS = {
...
function parseReport(text){
  ...
  // Narkotika dewasa
  var nDew = tDew.match(/klien narkotika[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i) ||
             tDew.match(/narkotika[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
  var nDewVal = nDew ? parseInt(nDew[1].replace(/[.,]/g,''), 10) : null;

  // Teroris dewasa (Klien Teroris)
  var terDew = tDew.match(/klien teroris[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i) ||
               tDew.match(/teroris[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
  var terDewVal = terDew ? parseInt(terDew[1].replace(/[.,]/g,''), 10) : null;
  ...
  // Narkotika anak
  var nAnak = tAnak.match(/klien narkotika[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i) ||
              tAnak.match(/narkotika[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
  var nAnakVal = nAnak ? parseInt(nAnak[1].replace(/[.,]/g,''), 10) : null;

  // Teroris anak
  var terAnak = tAnak.match(/teroris[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i);
  var terAnakVal = terAnak ? parseInt(terAnak[1].replace(/[.,]/g,''), 10) : null;

  // Gabungan narkotika & teroris
  ...
  // PiB anak
  d.pib_anak = extractBlockTotal(tAnak, 'pib');

  // AKOT (anak)
  d.akot = extractNum(tAnak, [
    /akot[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i,
    /^akot\s*[:\-]?\s*([\d.,]+)/im
  ]);

  // Diversi (anak)
  d.diversi = extractNum(tAnak, [
    /diversi[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i,
    /^diversi\s*[:\-]?\s*([\d.,]+)/im
  ]);

  // Klien Anak Sekolah
  d.sekolah = extractNum(tAnak, [
    /klien anak sekolah[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i,
    /klien anak sekolah\s*[:\-]?\s*([\d.,]+)/i,
    /sekolah[\s\S]{0,120}jumlah\s*[=:]\s*([\d.,]+)/i
  ]);
...
function generateLaporan(){
  ...
  laporan_count++;
  document.getElementById('ov-laporan-hari').textContent=laporan_count;
  var totalKlien=(n(smr.dewasa)+n(smr.anak))+(n(bpp.dewasa)+n(bpp.anak))+(n(trk.dewasa)+n(trk.anak));
  if(totalKlien>0){
    document.getElementById('ov-total-klien').textContent=fmt(totalKlien);
    var hk=document.getElementById('ov-total-klien-hero');if(hk)hk.textContent=fmt(totalKlien);
  }
  syncInfTotalKlien();
...
/* ===== LAPORAN BULANAN — GDRIVE ===== */
/* Daftar 6 key folder laporan bulanan */
...
function updateAllDriveBtnState() {
  var allSet = FOLDER_KEYS.every(function(k){ return !!DRIVE_LINKS[k]; });
  // Update status & tombol di setiap card (smr/bpp/trk sekarang satu tampilan bersama)
  ['smr','bpp','trk'].forEach(function(id){
    var btn = document.getElementById('btn-drive-' + id);
    var status = document.getElementById('status-drive-' + id);
    if (!btn || !status) return;
    if (allSet) {
      btn.disabled = false;
      status.innerHTML = '<span style="color:var(--green);font-weight:700;">✓ Drive terhubung</span>';
    } else {
      btn.disabled = false; // tetap aktif, folder individual yang belum diatur akan ditangani
      status.innerHTML = '<span style="color:#f59e0b;font-weight:700;">⚠ Sebagian link belum diatur</span>';
    }
  });
  // Update status per folder row
  FOLDER_KEYS.forEach(function(k){
    var s = document.getElementById('status-folder-' + k);
    if (s) {
      if (DRIVE_LINKS[k]) {
        s.innerHTML = '<span style="color:var(--green);font-size:10px;">✓</span>';
      } else {
        s.innerHTML = '<span style="color:#f59e0b;font-size:10px;">⚠</span>';
      }
    }
  });
}
...
function openFolder(key) {
  var link = DRIVE_LINKS[key];
  if (!link) { showToast('Link folder ini belum diatur. Admin dapat mengisi via ⚙️ Edit Link Drive.', 'error'); return; }
  ...
  if (currentUser) logActivity('Buka folder ' + key + ' oleh ' + currentUser.nama);
}

function openDriveLinkModal() {
  if (currentRole !== 'admin') { showToast('Hanya admin yang dapat mengubah link Drive.', 'error'); return; }
  FOLDER_KEYS.forEach(function(k){
    var el = document.getElementById('drive-input-' + k);
    if (el) el.value = DRIVE_LINKS[k] || '';
  });
...
function saveDriveLinks() {
  FOLDER_KEYS.forEach(function(k){
    var el = document.getElementById('drive-input-' + k);
    if (el) DRIVE_LINKS[k] = el.value.trim();
  });
  updateAllDriveBtnState();
  closeModal('modal-drive-link');
  showToast('Link Google Drive berhasil diperbarui! ✓', 'success');
