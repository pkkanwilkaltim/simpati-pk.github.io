...
function initApp() {
  ...
  renderUserTable();
  renderActivityLog();
  ['smr','bpp','trk'].forEach(updateDriveBtnState);
...
/* ===== LAPORAN BULANAN — GDRIVE LINKS ===== */
var DRIVE_LINKS = { smr: '', bpp: '', trk: '' };

function updateDriveBtnState(id) {
  var link = DRIVE_LINKS[id];
  ...
  if (link) {
    ...
    status.innerHTML = '<span class="lapbul-has-link">✓ Drive terhubung</span>';
  } else {
    btn.disabled = true;
    status.innerHTML = '<span class="lapbul-no-link">⚠ Link belum diatur</span>';
  }
}

function openDriveMain(id) {
  var link = DRIVE_LINKS[id];
  if (!link) { showToast('Link Google Drive belum diatur.', 'error'); return; }
  window.open(link, '_blank');
}

function openFolderLink(id) {
  var link = DRIVE_LINKS[id];
  if (!link) { showToast('Link Google Drive belum diatur. Admin dapat mengaturnya via ⚙️ Kelola Link Drive.', 'error'); return; }
  window.open(link, '_blank');
}

function openDriveLinkModal() {
  if (currentRole !== 'admin') { showToast('Hanya admin yang dapat mengatur link Drive.', 'error'); return; }
  document.getElementById('drive-input-smr').value = DRIVE_LINKS.smr;
  document.getElementById('drive-input-bpp').value = DRIVE_LINKS.bpp;
  document.getElementById('drive-input-trk').value = DRIVE_LINKS.trk;
  openModal('modal-drive-link');
}

function saveDriveLinks() {
  var smr = document.getElementById('drive-input-smr').value.trim();
  var bpp = document.getElementById('drive-input-bpp').value.trim();
  var trk = document.getElementById('drive-input-trk').value.trim();
  DRIVE_LINKS.smr = smr; DRIVE_LINKS.bpp = bpp; DRIVE_LINKS.trk = trk;
  ['smr', 'bpp', 'trk'].forEach(updateDriveBtnState);
  closeModal('modal-drive-link');
  showToast('Link Google Drive berhasil disimpan! ✓', 'success');
  logActivity('Update link Drive Bapas oleh ' + currentUser.nama);
}
