...
function initApp() {
  ...
  // Tampilkan data JFT: — jika belum pernah generate, atau data terakhir jika sudah
  _displayJft(_lastJftData);
  syncOvJft();
...
function syncOvJft() {
  ...
  // Update dashboard hanya jika sudah pernah generate (ada _lastJftData)
  // Jika belum generate, biarkan tetap tampil — 
  if (_lastJftData !== null) {
    _lastJftData = {kw:kw, smr:smr, bpp:bpp, trk:trk, tot:tot};
    _displayJft(_lastJftData);
    try { localStorage.setItem('lastJftData', JSON.stringify(_lastJftData)); } catch(e) {}
  }
...
/* ===== DATA JFT — hanya tampil setelah generate laporan ===== */
var _lastJftData = null; // null = belum pernah generate
try {
  var _savedJft = localStorage.getItem('lastJftData');
  if (_savedJft) _lastJftData = JSON.parse(_savedJft);
} catch(e) {}

function _displayJft(data) {
  // data = {kw, smr, bpp, trk, tot} atau null
  var dash = '—';
  var kw  = data ? data.kw  : dash;
  var smr = data ? data.smr : dash;
  var bpp = data ? data.bpp : dash;
  var trk = data ? data.trk : dash;
  var tot = data ? data.tot : dash;
  function setTxt(id, v) { var el = document.getElementById(id); if (el) el.textContent = v; }
  setTxt('ov-jft-kanwil', kw);
  setTxt('ov-jft-smr', smr);
  setTxt('ov-jft-bpp', bpp);
  setTxt('ov-jft-trk', trk);
  setTxt('ov-total-jft', tot);
...
function generateLaporan(){
  ...
  // Simpan data JFT ke state permanen dan update dashboard
  _lastJftData = {
    kw:  jft.kanwil_total,
    smr: jft.smr_total,
    bpp: jft.bpp_total,
