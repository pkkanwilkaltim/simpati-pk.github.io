...
function goPage(id, navEl) {
  ...
  if (id==='dashboard') { setTimeout(function(){ updateTotalKlienAktif(0); syncOvJft(); }, 50); }
...
function generateLaporan(){
  ...
  // Fallback: jika parseReport gagal ekstrak, ambil dari teks hasil laporan
  if(totalKlien===0){
    var mTotal=result.match(/II\.DATA KLIEN\s*:\s*([\d.,]+)\s*ORANG/i);
    if(mTotal){totalKlien=parseInt(mTotal[1].replace(/[.,]/g,''),10)||0;}
  }
  updateTotalKlienAktif(totalKlien);
  // Pastikan update terjadi setelah DOM render
  setTimeout(function(){ updateTotalKlienAktif(totalKlien); }, 100);
  syncInfTotalKlien();
